-- =============================================================================
-- v_inbox_threads: use REAL participant display name from profiles
-- (full_name -> name -> '—'). No hardcoded 'Korisnik'.
-- =============================================================================

-- Ensure profiles has name for fallback (some schemas may only have full_name)
...
-- Other participant: id + display name from profiles (full_name, then name, then '—')
other_party AS (
  SELECT
...
COMMENT ON VIEW public.v_inbox_threads IS 'Inbox threads: other_user_name from profiles (full_name, name, —). No hardcoded label.';
