-- =============================================================================
-- Auto-create contract when proposal gets accepted/hired
-- Trigger + ensure_contract_for_proposal + backfill
-- =============================================================================

-- 0) Allow 'hired' in proposals status (client UI may use it; ensure_contract accepts both)
ALTER TABLE public.proposals DROP CONSTRAINT IF EXISTS proposals_status_check;
ALTER TABLE public.proposals ADD CONSTRAINT proposals_status_check
  CHECK (status IN ('pending', 'accepted', 'hired', 'rejected', 'withdrawn', 'expired'));

-- 1) Unique constraint on contracts (job_id, freelancer_id) for ON CONFLICT
CREATE UNIQUE INDEX IF NOT EXISTS contracts_job_freelancer_uniq
  ON public.contracts (job_id, freelancer_id);

-- 2) ensure_contract_for_proposal: SECURITY DEFINER, handles both accepted/hired
...
BEGIN
  ...
  -- Insert or get existing (ON CONFLICT returns existing row id)
...
-- 3) Trigger function: fire when status changes to accepted/hired
...
-- 4) Backfill: ensure contract for all accepted/hired proposals that don't have one
DO $$
DECLARE
  r RECORD;
  v_contract_id bigint;
