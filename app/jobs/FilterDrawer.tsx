...
export default function FilterDrawer({ open, onClose, children }: Props) {
  ...
  return (
    <>
      {/* Overlay — blur + darken, click to close */}
      <div
        role="presentation"
        aria-hidden
      ...
      <div
        role="dialog"
        aria-label="Filteri poslova"
        className={`fixed top-0 right-0 h-full w-[320px] max-w-[90vw] bg-white z-50 shadow-xl flex flex-col transition-transform duration-200 ease-out ${
          slideIn ? "translate-x-0" : "translate-x-full"
        }`}
        onClick={(e) => e.stopPropagation()}
      >
        <div className="shrink-0 flex items-center justify-between px-4 py-3 border-b border-gray-200">
          <span className="text-sm font-semibold text-gray-700">Filteri</span>
          <button
            type="button"
            onClick={onClose}
